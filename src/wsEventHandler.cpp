...
void wsEventHandler(AsyncWebSocket *server, AsyncWebSocketClient *client, const AwsEventType type, void *arg, uint8_t *data,
                    size_t len) {
    if (type == WS_EVT_DATA) {
    ...
    Serial.println(dataBuffer);
        ...
        DeserializationError error = deserializeJson(recievedJson, reinterpret_cast<char *>(data), len);
        if (error) {
